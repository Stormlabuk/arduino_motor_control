...
void act_cb(const std_msgs::UInt32 &cmd_msg) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // sprintf(loginfo_buffer, "extracted at i=%d: %02X", i,
        // extractedBytes[i]); nh.loginfo(loginfo_buffer);
    ...
    act0.writeMicroseconds(step_inputs[3]);
    act1.writeMicroseconds(step_inputs[2]);
    act2.writeMicroseconds(step_inputs[1]);
    act3.writeMicroseconds(step_inputs[0]);
}
